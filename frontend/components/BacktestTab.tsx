...
export default function BacktestTab({ 
  ...
  // Strategy configurator form values
  ...
  // Backtest simulation tracking states
  ...
  const handleTriggerBacktest = async () => {
    ...
    try {
      ...
      // Store results for display (would need to add state for this)
      setTimeout(() => {
        clearInterval(progressInterval);
        setProgressPercent(100);
        setRunningBacktest(false);
        setBacktestCompleted(true);
        triggerToast('success', 'Backtest Completed', `Sharpe: ${result.performance.sharpe_ratio} | Return: ${result.capital.total_return}%`);
      }, 1500);
  ...
  return (
    <div className="flex flex-col gap-6 w-full">
      ...
      <div className="bg-[#1E293B] border border-[#475569] p-5 rounded-xl flex flex-col gap-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 pb-2">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="h-10 bg-[#0F172A] border border-[#475569] rounded-lg px-3 text-xs flex items-center gap-1.5 font-mono text-white select-none">
              <span className="w-2 h-2 rounded-full bg-[#3B82F6]" />
              NVDA, AAPL, MSFT, BTC, ETH
            </div>
          </div>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <input 
              ...
              disabled
              className="h-10 bg-[#0F172A]/70 border border-[#475569]/50 rounded-lg px-3 text-xs text-slate-400 font-mono focus:outline-none cursor-not-allowed select-all"
            />
          </div>
      ...
      {/* RECONSTRUCT ACTIONS PROGRESS DIALOG BAR */}
      {runningBacktest && (
        <div className="bg-[#1E293B] border border-[#3B82F6] p-6 rounded-xl flex flex-col gap-3 shadow-xl animate-pulse">
      ...
      {/* BACKTEST SIMULATION RESULTS OUTPUT */}
      {backtestCompleted && (
        <div className="flex flex-col gap-6 animate-fade-in">
          ...
          {/* Cards metrics table */}
          <div className="grid grid-cols-2 lg:grid-cols-6 gap-3.5 font-mono text-xs">
            {/* Box 1 */}
            <div className="bg-[#1E293B] border border-[#475569] p-3.5 rounded-xl flex flex-col gap-1 justify-center relative group">
              ...
              <span className="text-lg font-black text-[#10B981]">+42.70%</span>
              <div className="flex items-center gap-0.5 pt-1">
                {Array.from({ length: 5 }).map((_, i) => (
                  <span key={i} className="w-2 h-1 bg-[#10B981] rounded-sm" />
                ))}
              </div>
            </div>
            ...
            <div className="bg-[#1E293B] border border-[#475569] p-3.5 rounded-xl flex flex-col gap-1 justify-center relative group">
              ...
              <span className="text-lg font-black text-white">+31.25%</span>
              <div className="flex items-center gap-0.5 pt-1">
                {Array.from({ length: 5 }).map((_, i) => (
                  <span key={i} className={`w-2 h-1 rounded-sm ${i < 4 ? 'bg-[#10B981]' : 'bg-slate-700'}`} />
                ))}
              </div>
            </div>
            ...
            <div className="bg-[#1E293B] border border-[#475569] p-3.5 rounded-xl flex flex-col gap-1 justify-center relative group">
              ...
              <span className="text-lg font-black text-white">-12.42%</span>
              <div className="flex items-center gap-0.5 pt-1">
                {Array.from({ length: 5 }).map((_, i) => (
                  <span key={i} className={`w-2 h-1 rounded-sm ${i < 3 ? 'bg-[#EF4444]' : 'bg-slate-700'}`} />
                ))}
              </div>
            </div>
            ...
            <div className="bg-[#1E293B] border border-[#475569] p-3.5 rounded-xl flex flex-col gap-1 justify-center relative group">
              ...
              <span className="text-lg font-black text-[#3B82F6]">2.14</span>
              <div className="flex items-center gap-0.5 pt-1">
                {Array.from({ length: 5 }).map((_, i) => (
                  <span key={i} className="w-2 h-1 bg-[#3B82F6] rounded-sm" />
                ))}
              </div>
            </div>
            ...
            <div className="bg-[#1E293B] border border-[#475569] p-3.5 rounded-xl flex flex-col gap-1 justify-center relative group">
              ...
              <span className="text-lg font-black text-white">64.30%</span>
              <div className="flex items-center gap-0.5 pt-1">
                {Array.from({ length: 5 }).map((_, i) => (
                  <span key={i} className={`w-2 h-1 rounded-sm ${i < 4 ? 'bg-[#10B981]' : 'bg-slate-700'}`} />
                ))}
            ...
            <div className="bg-[#1E293B] border border-[#475569] p-3.5 rounded-xl flex flex-col gap-1 justify-center relative group">
              ...
              <span className="text-lg font-black text-white">247 orders</span>
              <div className="flex items-center gap-0.5 pt-1">
                <span className="text-[10px] text-[#94A3B8]">Analyzed period</span>
              </div>
            </div>
          </div>

          {/* SVG STRATEGY COMPARISON CHART REPRESENTATIONS */}
          <div className="bg-[#1E293B] border border-[#475569] p-4 rounded-xl flex flex-col gap-4">
            ...
            <div className="relative w-full h-[220px] bg-[#0F172A] border border-[#475569] rounded-lg p-2">
              ...
              {/* Multiple paths rendering Strategy (Blue solid) vs Benchmark (Green dashed) */}
              <svg viewBox="0 0 600 200" preserveAspectRatio="none" className="w-full h-full">
                {/* 1. SOLID Blue curve (Strategy) */}
                <path 
                ...
                {/* 2. DASHED Green curve (S&P 500 Benchmark) */}
                <path 
          ...
          <div className="bg-[#1E293B] border border-[#475569] p-4 rounded-xl flex flex-col gap-4">
            ...
            {/* Selected Heat block details display */}
            {selectedHeatBlock && (
